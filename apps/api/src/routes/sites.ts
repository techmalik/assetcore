...
sitesRouter.use(requireAuth, requireOrg)

const ALLOWED = ['name', 'code', 'region', 'lat', 'lng']
...
sitesRouter.post('/sites', async (req, res) => {
  ...
  const { name, code, region, lat, lng } = parsed.data

  const row = await withOrgContext(claimsFromReq(req), async (c) => {
    const { rows } = await c.query(
      `insert into public.sites (org_id, name, code, region, lat, lng)
       values (current_org_id(), $1, $2, $3, $4, $5) returning *`,
      [name, code ?? null, region ?? null, lat ?? null, lng ?? null]
    )
    const site = rows[0]
