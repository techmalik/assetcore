// The Compliance feature body, extracted from pages/Compliance.jsx so it can be
// mounted twice: as the standalone /compliance page, and as the Maintenance
// page's Compliance tab — which used to render a hardcoded array of seven fake
// licences, fake status counters and a dead "Renew" button, all while this real
// licence register already existed one nav item away.
//
// Owns its own loading and capability gates so both mount points behave
...
export default function CompliancePanel({ embedded = false, selectedId = null, onCounts = null }) {
  ...
  // Report up so a host can badge a tab without re-fetching. `alerts` uses the
  // same expiring + due_soon + expired definition as the Dashboard's
  // "Compliance Alerts" KPI, so the two never disagree.
  const alertCount = counts.expiring + counts.due_soon + counts.expired
  useEffect(() => {
    if (!onCounts) return
    onCounts({ total: counts.total, alerts: alertCount, expired: counts.expired })
  }, [onCounts, counts.total, alertCount, counts.expired])

  // Deep-link landing (?id=): open that licence's detail panel, clearing any
  // filter that would otherwise hide the row.
  useEffect(() => {
    if (!selectedId || !licences.length) return
    const target = licences.find(l => l.id === selectedId)
    if (!target) return
    setView('licences')
    setFilter('all')
    setKindFilter('')
  ...
  return (
    <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
          {/* Header */}
          <div style={{padding: embedded ? '10px 24px 12px' : '14px 24px 12px',borderBottom:'var(--bdr)',background:'var(--n0)',flexShrink:0}}>
