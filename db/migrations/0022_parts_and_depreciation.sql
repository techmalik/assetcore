-- ============================================================================
-- 0022_parts_and_depreciation
--
-- Spare parts inventory, what a technician actually records on a job, and a
-- depreciation subledger layered on top of 0015's derived net book value.
--
-- On depreciation, the two branches solved different halves of the same
-- problem and both halves are worth keeping:
--
--   0015 derives nbv_cents monthly from a resolved policy. It is always
--   current, needs no user action, and correctly reports NULL rather than 0
--   when it cannot know. That stays exactly as it is, and remains the default.
--
--   This migration adds the subledger: one schedule per asset, annual entries,
--   and an explicit posting step. That is what an auditor asks for — "show me
--   the charge for 2024 and who posted it" — which a recomputed current figure
--   can never answer, because it keeps no history.
--
--   assets.nbv_source arbitrates: 'schedule' once a period has been posted,
--   'derived' otherwise. recompute_asset_depreciation_for() is taught to
--   leave posted assets alone, so the two never fight over the same column.
-- ============================================================================
...
--
-- Replaces the free-text work_orders.parts jsonb blob with lines that point at
-- real stock. That column stays until every read path has moved.
-- ----------------------------------------------------------------------------
...
--
-- Entries are ANNUAL periods: that is the granularity a fixed asset register
-- is reported at, and it keeps the maths verifiable by hand against the
-- client's existing spreadsheet. 0015's monthly derived figure covers the
-- "what is it worth today" question in between.
-- ----------------------------------------------------------------------------

-- 0015 allowed none/straight_line/declining_balance. The subledger also does
-- sum-of-years' digits, so the shared column has to admit it.
do $$
begin
  if exists (select 1 from pg_constraint where conname = 'assets_depreciation_method_check') then
    alter table public.assets drop constraint assets_depreciation_method_check;
  end if;
...
-- Which of the two owns assets.nbv_cents. 'derived' is 0015's monthly figure
-- and the default for every asset; 'schedule' means a posted period owns it
-- and the nightly recompute must keep its hands off.
alter table public.assets
  add column if not exists nbv_source text not null default 'derived';
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'assets_nbv_source_check') then
    alter table public.assets add constraint assets_nbv_source_check
      check (nbv_source in ('derived','schedule'));
  end if;
end $$;

comment on column public.assets.nbv_source is
  'Which writer owns nbv_cents: ''derived'' = recompute_asset_depreciation_for() (0015), '
  '''schedule'' = a posted depreciation_entries period (0022). Never client-writable.';

-- ----------------------------------------------------------------------------
-- 6. recompute_asset_depreciation_for v2
--
-- Same maths as 0015, with two changes: it now yields to a posted subledger,
-- and it resolves policy through the asset's category before falling back to
-- the org default, so 0021's category defaults are actually used.
-- ----------------------------------------------------------------------------
...
begin
  select a.id, a.purchase_value_cents, a.purchase_date, a.install_date, a.nbv_source,
         ...
         coalesce(a.declining_rate_pct,
                  ...
                  20)                                                   as eff_rate,
         coalesce(c.salvage_rate_pct,
                  nullif(o.settings->'depreciation'->>'salvageRatePct', '')::numeric, 0) as eff_salvage_pct,
  ...
  -- A posted subledger period owns this asset's book value. Recomputing over
  -- it would silently replace a figure someone signed off with one nobody did.
  if v_asset.nbv_source = 'schedule' then
    return;
  end if;
  ...
  -- Unknown, not zero: null renders as "—", zero claims a fully written-down
  -- asset, which is a much more confident statement than we can make.
  ...
  elsif v_method = 'sum_of_years_digits' then
    -- Front-loaded like declining balance but on the depreciable base, and it
    -- terminates on its own. n(n+1)/2 is the denominator; the numerator is
    -- the years remaining at the start of the current one.
    v_depreciable := v_purchase - v_salvage;
    declare
      v_years   numeric := least(v_months::numeric / 12.0, v_life);
      v_n       numeric := ceil(v_life);
      v_syd     numeric := (v_n * (v_n + 1)) / 2;
      v_elapsed numeric := floor(v_years);
      v_consumed numeric := 0;
      i numeric;
    begin
      for i in 0 .. (v_elapsed - 1) loop
        v_consumed := v_consumed + (v_n - i) / v_syd;
      end loop;
      -- Part-year within the current period, charged pro rata.
      v_consumed := v_consumed + ((v_n - v_elapsed) / v_syd) * (v_years - v_elapsed);
      v_accum := round(v_depreciable * least(1.0, greatest(0.0, v_consumed)))::bigint;
    end;
...
-- ----------------------------------------------------------------------------
-- 7. PM schedules — checklist templates and custom intervals
--
-- pm_tasks.checklist_results has existed since 0001 with no checklist
-- definition behind it, so there was nothing to record results against. The
-- template lives on the schedule and is copied onto every task it generates.
-- ----------------------------------------------------------------------------
...
-- When set, overrides `frequency` — a 45-day cycle has no enum value.
alter table public.pm_schedules
  add column if not exists interval_days int;
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'pm_schedules_interval_days_check') then
    alter table public.pm_schedules add constraint pm_schedules_interval_days_check
...
-- ----------------------------------------------------------------------------
-- 8. Low stock notifications (node-cron: 06:30 daily)
--
-- Routed through notify_role_holders() (0014) rather than inserting into
-- notifications directly, so it respects notification_preferences and dedupes
-- — the fix 0016 applied to every other producer.
-- ----------------------------------------------------------------------------
...
begin
  for v_part in
    select p.id, p.org_id, p.part_number, p.name, p.quantity_in_stock, p.reorder_level, p.unit
  ...
  loop
    ...
    v_qty   := rtrim(to_char(v_part.quantity_in_stock, 'FM999999990.99'), '.');
    v_level := rtrim(to_char(v_part.reorder_level,     'FM999999990.99'), '.');

    perform public.notify_role_holders(
      v_part.org_id, null,
      array['owner','ops_manager','maint_engineer'],
      null,
      'low_stock',
      'Low stock: ' || v_part.name,
      v_part.part_number || ' is down to ' || v_qty || ' ' || v_part.unit
        || ' (reorder at ' || v_level || ').',
      'spare_part', v_part.id,
      -- One notification per part per day, not one per cron run.
      'low_stock:' || v_part.id::text || ':' || current_date::text
    );
    v_count := v_count + 1;
  end loop;
  return v_count;
